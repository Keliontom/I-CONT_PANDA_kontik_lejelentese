...
class Icont_sheet():
    def __init__(self):
        ...
        self.google_account = gspread.service_account(filename="pandalejelentes-0cf5d8fb52cc.json") # - egy meghatározott email cimmel lehet módosítani a táblázatokat.
    ...
    def opensheet(self, sheetname, worksheet):
        ...
        try:
            #self.sheet = self.google_account.open(sheetname)
    ...
    def bild_dictionary(self, row_key, row_value):
        ...
        #row_dict = {key: value for key, value in zip(row_key, row_value)}
        #row_dict = {row_key[i]:row_value[i] for i in range(len(row_key))}
        return row_dict
